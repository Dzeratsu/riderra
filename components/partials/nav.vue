<template>
  <header class="header" :class="{'menu-open': menu}">
    ...
    <nav class="nav-list">
      <nuxt-link to="/" class="nav-list__item" @click.native="scrollTo('#howWorks')"> {{ this.$tc('main.carousel') }}</nuxt-link>
      <nuxt-link to="/" class="nav-list__item" @click.native="scrollTo('#park')">{{this.$tc('main.autopark')}}</nuxt-link>
      <nuxt-link to="/" class="nav-list__item" @click.native="scrollTo('#reviews')">{{this.$tc('main.reviews')}}</nuxt-link>
    </nav>
    <tabs-nav></tabs-nav>
    <div class="header__right">
      <a href="tel:+78122004946" class="header__tel">+7-812-200-49-46</a>
<!--
      <a href="#" class="header__signin">{{$tc('main.utility.login')}}</a>
-->
    </div>
  ...
  export default {
    ...
    data(){
      return {
        mobileMenu: false,
        navList: [
          this.$tc('main.carousel'),
          this.$tc('main.autopark'),
          this.$tc('main.reviews')
        ],
        ua: '',
        langData: {
          class: ''
    ...
    beforeMount(){
      ...
      if (!this.$store.getters.checkCoords) {
        var ls = localStorage.getItem("coord");
        if (ls!=null) {
          this.$store.state.coordinates=JSON.parse(ls);
        }

        if (this.$store.state.distance=="" && localStorage.getItem("distance")!=null) {
          this.$store.commit('setDistance', localStorage.getItem("distance"));
        }
      }
      if (this.$store.state.ivcardoPrices==null && localStorage.getItem("lastPrices")) {
        this.$store.commit("setPriceLoaded",true);
        this.$store.state.ivcardoPrices=JSON.parse(localStorage.getItem("lastPrices"));
      }
